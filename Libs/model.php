<?php 
	...
	**/
	...
	function query($req){
		global $db;
		$key = isset($req['key']) ? $req['key'] : 'id';
		$type = isset($req['type']) ? $req['type'] : 'INSERT';
		if ($type == 'INSERT') {
			$sql = 'INSERT INTO '.$req['table'].' SET '. $req['sql'];
			$action = 'insert';
		} elseif ($type == "UPDATE") {
			$sql = 'UPDATE '.$req['table'].' SET '.$req['sql'] . ' WHERE '.$key.'=:'.$key;
			$id = $req['param'][':'.$key];
			$action = 'updated';
		}

		$pre = $db->prepare($sql);
		$pre->execute($req['param']);
	...
	function connect($arg = "local") {
		...
		try {
			...
			$pdo = new PDO(
				$data['driver'] . ':host='.$data['host'].';dbname='.$data['base'].';',
				$data['user'],
				$data['pass'],
